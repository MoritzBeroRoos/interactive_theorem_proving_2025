...
theorem FACT_correct (n₀ : ℕ) :
    ...
    (  Stmt.assign "i" (fun s ↦ 0);
       Stmt.assign "r" (fun s ↦ 1);
       Stmt.invWhileDo (fun s ↦ s "r" = fact (s "i") ∧ s "n" = n₀) (fun s ↦ s "i" ≠ s "n")
      (Stmt.assign "i" (fun s ↦ s "i" + 1);
       Stmt.assign "r" (fun s ↦ s "r" * s "i")))
       {* fun s ↦ s "r" = fact n₀ *}
    from by
      vcg
      all_goals aesop (add norm fact) <;> ac_rfl
...
theorem loop_intro {P S} (h : {* P *} (S) {* P *}) :
  ...
  generalize eq_Ss: (S.loop, s) = Ss at hloop
  induction hloop generalizing S s with
  | loop0 S s' => aesop
  | loop S' s' t u hSst hLoopStu hind hind2 =>
    have : P t := by aesop
    #check hind2 --why isn't it forall quantified over S here?? it shows like that in the infoview?
    have : P u := by aesop
    --exact this
  | _ => aesop
